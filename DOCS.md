...
2.  Start the Add-on.
3.  Open the Web UI by clicking **"Open Web UI"** or looking for the sidebar tab.
4.  In the blue GRUB installer menu, select the third option: **`Installation - Install Android-x86 to harddisk`** and press Enter.
5.  Select **`Create/Modify partitions`** and press Enter.
6.  When asked: *Do you want to use GPT?*, select **`No`**.
7.  In the `cfdisk` partition manager screen:
    *   Highlight the **`Free Space`** area.
    *   Select **`New`** -> **`Primary`** -> press Enter to accept the default size.
    *   Select **`Bootable`** (you will see `Boot` appear under the *Flags* column).
    *   Select **`Write`** -> type **`yes`** -> press Enter.
    *   Select **`Quit`**.
8.  Select the newly created partition **`sda1`** and press Enter.
9.  Select **`ext4`** as the filesystem.
10. Select **`Yes`** to format the partition.
11. Select **`Yes`** to install the **GRUB bootloader**.
12. Select **`Yes`** to make the **`/system` directory read-write**.
13. Once the progress bar reaches 100%, select **`Run Android-x86`** to verify it boots.
14. **Crucial:** Once the Android home screen displays, go back to the Home Assistant Add-on dashboard and **STOP** the Add-on.
